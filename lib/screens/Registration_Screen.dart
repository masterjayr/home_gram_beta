...
class RegistrationScreen extends StatefulWidget {
  ...
  final BaseAuth auth;
  @override
  _RegistrationScreenState createState() => _RegistrationScreenState(auth: auth);
}
...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  bool validateAndSave() {
    ...
    if(role != null){
      if (form.validate()) {
      print('The email is $_email, and password is $_password, name is $_name, role is $role');
      form.save();
      return true;
    } else {
      return false;
      ...
      Fluttertoast.showToast(msg: 'Choose a Role either Landlord or tenant');
      return false;
    }
  ...
  void validateAndSubmit(BuildContext context) async {
    ...
    if (validateAndSave()) {
      try {
        ...
        final result =
            await auth.createUserWithEmailAndPassword(_email, _password, _name, phoneNo, role);
        print('userId: $result');
        if (result.user != null) {
          setState(() {
            isLoading = false;
          });
          String photoUrlFromSignUp = prefs.getString('photoUrlFromSignUp');
          await prefs.setString('uid', result.user.uid);
          await prefs.setString('email', result.user.email);
          await prefs.setString('photoUrl',
              result.user.photoUrl == null ? '' : photoUrlFromSignUp);
          await prefs.setString('displayName', _name);
          await prefs.setInt('phoneNo', phoneNo);
          await prefs.setString('phoneNo', role);
                        ...
                        child: Column(
                          children: <Widget>[
                            ...
                            TextFormField(
                                ...
                                obscureText: iconToggle ? false : true,
                                decoration: InputDecoration(
                                  suffixIcon: iconToggle == false ? IconButton(icon: Icon(MaterialCommunityIcons.eye_off),
                                  onPressed: (){
                                    setState(() {
                                      iconToggle = true;
                                    });
                                 ...
                                 IconButton(icon: Icon(MaterialCommunityIcons.eye),
                                  onPressed: (){
                                    setState(() {
                                      iconToggle = false;
                                    });
